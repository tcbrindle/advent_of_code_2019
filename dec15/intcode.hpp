...
public:
    ...
    std::vector<int64_t> run_until_input()
    {
        std::vector<int64_t> outputs;
        auto in = [] { return 0; };
        auto out = [&outputs] (auto val) { outputs.push_back(val); };
    ...
    constexpr int64_t run_until_output(int64_t input)
    {
        if (done_) {
            return input;
        }
        ...
        auto out_fn = [&output](int64_t val) {
            output = val;
        };
